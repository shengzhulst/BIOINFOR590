...
def generate_data_for_net(track,batch_size,index_part,chr_name):
	...
	for i in list(index_part)[track:track+batch_size]:
		empty_output=np.zeros((length_of_sequence_generate,6))
...
final_label_matrix_test=np.load('test_label_with_methylation.npy')
final_large_matrix_test=np.load('test_data_with_methylation.npy')
...
class Net(nn.Module):
  def __init__(self):
    ...
    self.cov1=nn.Conv1d(6,32,3,padding=1)
    self.bn1=nn.InstanceNorm1d(32)
...
for epoch in range(15):  # loop over the dataset multiple times
    ...
    with open('auc_roc_result_with_methylation.txt','a') as f:
        f.write(str(epoch)+'\n'+str(auroc.item())+'\t'+str(auprc.item())+'\n')
    print(auroc,auprc)
    torch.save(net.state_dict(),'./save_model_lr3_wei_3_with_meth_chr_4_20201213.pkl')
